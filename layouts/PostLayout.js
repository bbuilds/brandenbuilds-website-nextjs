...
export default function PostLayout({ frontMatter, next, prev, children }) {
  ...
  const { slug, date, title, tags, readingMinutes, featuredImage, fileName } =
    frontMatter;
      ...
      <article className={BlogStyles["blog-post"]}>
        ...
        >
          ...
          <footer>
            <EditDialog>
              <p>
                Notice something that needs to be changed? You can edit this
                post on Github!
              </p>
              <p>
                <a
            ...
            {(next || prev) && (
              <div className="py-4">
                {prev && (
                  <div>
                    <h2 className="text-small tracking-wide  uppercase">
                      Previous Article
                    </h2>

                    <Link href={`/${prev.slug}`} className="text-small">
                      {prev.title}
                    </Link>
                  </div>
                )}
                {next && (
                  <div className="mt-6">
                    ...
                    <Link href={`/${next.slug}`} className="text-small">
                      {next.title}
                    </Link>
                  </div>
                )}
