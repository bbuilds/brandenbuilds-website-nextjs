	...
	return (
		<ul className="flex">
			{socialItems.map((item) => (
				<li key={item.label} className="mr-6 md:mr-2">
					<a
						href={item.href}
						target="_blank"
						className="block transition duration-300 ease-in-out transform hover:-translate-y-1 text-bbuilds-teal w-6"
						rel="noopener noreferrer"
					>
						<div dangerouslySetInnerHTML={{ __html: item.icon }}></div>
						<span className="sr-only">{`Link to Branden Builds ${item.label}`}</span>
					</a>
				</li>
			))}
		</ul>
