	...
	return (
		<>
			...
			>
				{navItems.map((item) => {
					return (
						<li key={item.title} className={cn('relative group', navStyles['menu-item'])}>
							<Link href={`${item.href}`}>
								<a
									className={cn(
											...
											return (
												<li key={child.title} className="menu-item">
													<Link href={`${child.href}`}>
														<a className="flex text-bbuilds-teal py-2 px-4 transition duration-300 hover:bg-bbuilds-yellow hover:text-bbuilds-black">
															{child.title}
														</a>
													</Link>
												</li>
