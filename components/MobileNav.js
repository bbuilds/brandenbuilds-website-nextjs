...
export default function MobileNav({ navItems, toggleMenuState }) {
  return (
    <>
      <ul
        className={`nav-items flex flex-col px-4 pt-8 pb-20 overflow-y-scroll h-full`}
      >
        {navItems &&
          navItems.map((item) => {
            return (
              <li
                key={item.title}
                className="menu-item group mb-4"
                onClick={toggleMenuState}
              >
                <Link
                  href={item.href}
                  className="text-bbuilds-teal hover:text-bbuilds-yellow flex py-4 text-lg"
                >
                  {item.title}
                </Link>
                {item.children && (
                      ...
                      {item.children.map((child) => {
                        return (
                          <li key={child.title} className="menu-item">
                            <Link
                              href={`${child.href}`}
                              className="flex text-bbuilds-teal py-4 px-4 transition duration-300 hover:bg-bbuilds-yellow hover:text-bbuilds-black text-lg"
                            >
                              {child.title}
                            </Link>
                          </li>
